...
//#include "Tracking.h"  // <-- Add this include
...
void __stdcall ImageCallBackEx(unsigned char* pData, MV_FRAME_OUT_INFO_EX* pFrameInfo, void* pUser)
{
    if (pFrameInfo) {
        ...
        image.release(); // Deallocates memory, making the Mat empty
    }
...
void processBuffer1()
{
    while (!stopThreads.load()) {
        ...
        Mat image;
        {
            ...
            if (stopThreads.load() && buffer1.empty())
                break;
            if (!buffer1.empty()) {
                image = buffer1.front();
                buffer1.pop();
    ...
    while (!stopProcessing) {
        ...
        Mat image;
        {
         ...
         /*   if (stopProcessing ) break;*/
            if (stopProcessing.load() && buffer2.empty())
                break;
            if (!buffer2.empty()) {
                image = buffer2.front();
                buffer2.pop();
        ...
        //cout << "Number of contours detected: " << contours.size() << endl;
        ///////////////////////////////////////////////////////////////////////////////////////////tracking logic/////////////
         // Prepare a vector of bounding boxes from the contours
        //vector<cv::Rect> detectedParts;
        //for (const auto& cnt : contours) {
        //    cv::Rect bbox = boundingRect(cnt);
        //    // You might want to filter out small boxes
        //    if (bbox.area() > 500) {
        //        detectedParts.push_back(bbox);
        //    }
        //}
        //
        //// Call processFrame to update tracking and draw on the frame
        //processFrame(croppedImage, detectedParts);
...
void furtherProcessing(MainWindow* mainWindow, QLineEdit* countEdit, QLabel* statusLabel, QLineEdit* FRAME_COUNT)
{
    ...
    totalcounter.store(0);
    int counter_2 = 0;
    int maxUniqueInFirstImage = 0;
    int uniqueCountInPreviousImage = 0;
    vector<vector<ContourData>> prevContourData;
    vector<vector<ContourData>> changedContourData;
    setFrameCountValue(FRAME_COUNT);
    ...
    while (!stopProcessing) {
        ...
        Mat croppedImage;
        {
            unique_lock<mutex> lock(buffer3_mutex);
            buffer3_cv.wait(lock, [] { return !buffer3.empty() || stopProcessing; });
            if (stopProcessing.load() && buffer3.empty())
                break;
            if (!buffer3.empty()) {
                croppedImage = buffer3.front();
                buffer3.pop();
        ...
        if (counter_2 == 1) {
            maxUniqueInFirstImage = static_cast<int>(contours.size());
        }

        for (size_t contourIndex = 0; contourIndex < contours.size(); ++contourIndex) {
            const vector<Point>& contour = contours[contourIndex];
            Rect roi = defineROI(contour, croppedImage.cols, croppedImage.rows, 20);
            Point2f centroid = calculateCentroid(contour);
            double area = contourArea(contour);

            if (area > 500) {
                ContourData data;
                data.number = uniqueCountInPreviousImage + static_cast<int>(contourIndex) + 1;
                data.roi = roi;
                data.centroid = centroid;
                data.area = area;
                currentContourData.push_back(data);
        ...
        for (size_t contourIndex = 0; contourIndex < currentContourData.size(); ++contourIndex) {
            ...
            for (size_t i = 0; i < prevContourData.size(); ++i) {
                for (size_t j = 0; j < prevContourData[i].size(); ++j) {
                    ...
                    if (areRectanglesApproxEqual(prevData.roi, currentData.roi, tolerance)) {
                        if ((prevData.roi & currentData.roi).width > 0 && prevData.roi.contains(currentData.centroid)) {
                            if (prevData.centroid.y < currentData.centroid.y) {
                                clearNumberAndAdjustCounts(static_cast<int>(i), static_cast<int>(j), prevContourData);
                                isUnique = false;
                                break;
                            }
                        }
            ...
            if (isUnique) {
                processUniqueContour(currentData, croppedImage, contours, static_cast<int>(contourIndex));
                uniqueCount++;
            }
        }
        ...
        QString countText = QString::number(totalcounter);
        QMetaObject::invokeMethod(countEdit, "setText", Qt::QueuedConnection, Q_ARG(QString, countText));
        ...
        if (totalcounter >= FrameCountvalue) {
            ...
            // **Here is where we update the status**
            QMetaObject::invokeMethod(statusLabel, "setText", Qt::QueuedConnection, Q_ARG(QString, "Processing Complete"));

            break;
        }
        else if (totalcounter == (FrameCountvalue - 10)) {
            QMetaObject::invokeMethod(mainWindow, "RampUpDownMotor", Qt::QueuedConnection);
        }
