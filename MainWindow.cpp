...
void MainWindow::startProcessing()
{
    ...
    //QString speedValueString;
    //speedValueString = QString::number(speedValue);
    //QString startCommand = QString("$h%1").arg(speedValueString);
    ...
    if (speedValue > 0 && speedValue < 10) {
        speedValueString = QString::number(speedValue);
        speedValueString = "0" + speedValueString;
    }
    else if (speedValue >= 10) {
        speedValueString = QString::number(speedValue); //QString(QChar('A' + (speedValue - 10)));
    }
    else {
        return;
    }

    QString speedCommand = QString("$h%1").arg(speedValueString);
    sendCommandToArduino(speedCommand);
    ...
    processThread2 = QThread::create([this] { processBuffer2(); });
    processingThread = QThread::create([this] {
        furtherProcessing(this, ui->countEdit, ui->Status, ui->Frames);
        QMetaObject::invokeMethod(this, [this] {
            ui->Status->setText("Processing Complete");
            }, Qt::QueuedConnection);
        });
    enqueueThread->start();
    processThread1->start();
...
void MainWindow::stopProcessing_1()
{
    ...
    if (enqueueThread) {
        ...
        enqueueThread->quit();
        enqueueThread->wait(); // Wait for the thread to finish
        enqueueThread = nullptr; // Reset the pointer
    }
    if (processThread1) {
        connect(processThread1, &QThread::finished, processThread1, &QObject::deleteLater);
        processThread1->quit();
        processThread1->wait(); // Wait for the thread to finish
        processThread1 = nullptr; // Reset the pointer
    }
    if (processThread2) {
        connect(processThread2, &QThread::finished, processThread2, &QObject::deleteLater);
        processThread2->quit();
        processThread2->wait(); // Wait for the thread to finish
        processThread2 = nullptr; // Reset the pointer
    }
    if (processingThread) {
        ...
        processingThread = nullptr; // Reset the pointer
    }
}
...
void MainWindow::RampUpDownMotor()
{
    QString stopCommand = "$v01"; // Assuming $s is the stop command for your conveyor
    sendCommandToArduino(stopCommand);
}
